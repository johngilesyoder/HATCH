...
if ( class_exists( 'TribeEvents' ) ) {
	...
	function tribe_get_events_title( $depth = true ) {
		...
		if ( isset( $_REQUEST['tribe-bar-date'] ) && $wp_query->have_posts() ) {
			...
			$title = sprintf( __( 'Hatchings for %1$s - %2$s', 'tribe-events-calendar'), $first_event_date, $last_event_date );
		} elseif ( tribe_is_past() ) {
			$title = __( 'Past Hatchings', 'tribe-events-calendar' );
		}

		if ( tribe_is_month() ) {
			$title = sprintf(
				__( 'Hatchings for %s', 'tribe-events-calendar' ),
		...
		// day view title
		if ( tribe_is_day() ) {
			$title = __( 'Hatchings for', 'tribe-events-calendar' ) . ' ' .
